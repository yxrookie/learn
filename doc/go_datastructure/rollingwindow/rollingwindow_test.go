...
func TestNewRollingWindow(t *testing.T) {
	...
	assert.Panics(t, func() {Newrollingwindow(0, time.Second)})
}
...
func TestRollingWindowAdd(t *testing.T) {
	...
	// sleep time should be less than interval, and make the bucket change again
	time.Sleep(time.Millisecond * 20)
	r.Add(7)
	r.Add(8)
	r.Add(9)
	assert.Equal(t, []float64{1, 5, 24}, listBuckets())
	time.Sleep(time.Millisecond * 100)
	r.Add(10)
	r.Add(20)
	r.Add(30)
	assert.Equal(t, []float64{0, 0, 60}, listBuckets())
